...
class Console:
    ...
    def __init__(self, retro):
        ...
        def _event_proc():
            ...
            while True:
                event_ = self.event.find_event('console')['timer']
                etype_ = list(event_)[0]
                if etype_ == 'event':
                    for ev in retro.events('console'):
                        if event_['console'] == list(ev)[0]:
                            self._event.send_event(ev[event_['console']])
                elif etype_ == 'exec':
                    self.exec_(event_)
                else:
                    assert False
        ...
        self.event.register('console', _event_proc)
